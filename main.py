...
class bot(BaseBot):
    ...
    async def on_chat(self, user: User, message: str) -> None:
            ...
            if message.startswith("!gold") and user.username == "raymg":
                  wallet = (await self.highrise.get_wallet()).content
                  await self.highrise.chat(f"The bot wallet contains {wallet[0].amount} {wallet[0].type}")
            if message.lower().startswith("!gme ") and user.username=="raymg":
                try:
            ...
            if message.lower().startswith("!gall ") and user.username == "raymg":
              parts = message.split(" ")
              if len(parts) != 2:
